...
node {	
	...
	stage('Build Images'){
		dir("${env.WORKSPACE}") {
			...
			sh 's2i build --incremental=true --exclude="" https://github.com/ceskaexpedice/kramerius.git harbor.nkp.cz:30004/library/kramerius-builder:latest kramerius -r v5.4.3_NKP-dnnt'
		}
	}
